    ...
    if(F == 0) {
       if (D != 4 & A > 0) { # if not 4th down and >0 yards to end zone
          ...
          if (play_type == "P") {
             ...
             time_elapsed = rnorm(1, 22/60, 4/60) 
             ...
             if (pos_neg == "POS") {
                yards_gained = rlnorm(1, meanlog = 1.454, sdlog = .907) 
             } else if (pos_neg == "NEG") {
                yards_gained = -rlnorm(1, meanlog = .874, sdlog = .822)
             }
             
             time_elapsed = rnorm(1, 12/60, 4/60) 
          }
          ...
          if (A <= 30) { #if within 30 yards of end zone, simulate field goal
             ...
             drive_result$end_time <- B - rnorm(1, 8/60, 3/60)
             is_not_done <- FALSE
             ...
             drive_result$end_yard <- ifelse(A - punt_yards <= 0,
                                             25,
                                             A - punt_yards)
             drive_result$end_time <- B - rnorm(1, 14/60, 3/60)
       ...
       if (A > 0) { # if >0 yards to end zone
          ...
          if (play_type == "P") {
             ...
             time_elapsed = rnorm(1, 17/60, 3/60) 
            ...
            if (pos_neg == "POS") {
             ...
             time_elapsed = rnorm(1, 12/60, 3/60) 
            ...
            if (pos_neg == "POS") {
             ...
             time_elapsed = rnorm(1, 14/60, 4/60)
          }
          ...
          if(play_type == "DP" & pos_neg == "POS") {
             turnover <- sample.int(2, size = 1, 
                                    prob = c(.855, .145)) - 1
             D = ifelse(turnover == 1, 0, D)
          } else if(play_type == "P" & pos_neg == "POS") {
             turnover <- sample.int(2, size = 1, 
                                    prob = c(.96, .04)) - 1
             D = ifelse(turnover == 1, 0, D)
          }
